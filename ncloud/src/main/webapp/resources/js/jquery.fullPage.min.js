	...
	a.fn.fullpage = function (c) {
		...
		function ea(b) {
			...
			if (!Q(b.target) && (b = a(".fp-section.active"), !t && !q))
				if (d = R(d), y = d.y, B = d.x, b.find(".fp-slides").length && Math.abs(C - B) > Math.abs(z - y))
					Math.abs(C - B) >
		...
		function L(b, c) {
			...
			b === w || e.hasClass("active") ? jQuery(e, c) : l(e, function () {
				jQuery(e, c)
			})
		}
		function jQuery(a, c) {
		...
		function (b) {
			if (b) {
				if (a.fn.fullpage.setMouseWheelScrolling(!0), M || ca)
					MSPointer = aa(), a(document).off("touchstart " + MSPointer.down).on("touchstart " + MSPointer.down, fa), a(document).off("touchmove " + MSPointer.move).on("touchmove " + MSPointer.move, ea)
			} else if (a.fn.fullpage.setMouseWheelScrolling(!1), M || ca)
				MSPointer = aa(), a(document).off("touchstart " + MSPointer.down), a(document).off("touchmove " + MSPointer.move)
		};
		...
		a(this).length ? (h.css({
			...
			}), h.addClass("fullpage-wrapper")) : console.error("Error! Fullpage.js needs to be initialized with a selector. For example: jQuery('#myContainer').fullpage();");
